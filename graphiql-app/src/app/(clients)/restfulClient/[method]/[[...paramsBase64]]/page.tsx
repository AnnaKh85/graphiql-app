...
export default function RestfulClientPage({
  ...
  params: { method: string; paramsBase64: string[] };
}) {
  ...
  const [visibleEditHeadersModal, setVisibleEditHeadersModal] =
  ...
  function saveHeaders(headers: HttpHeader[]) {
    setHeaders(headers);
    router.push(
      urlBuildService.buildUrl_fromClient(
        path,
        params.paramsBase64,
        requestType,
        requestUrl,
  ...
  function renderHeadersCount() {
    return (
      <strong>
        <small>
          {t("headers_of_cnt")}: [{headers.length}]
        </small>
      </strong>
    );
  }
  ...
  function renderEpQueryParamsCount() {
    return (
      <strong>
        <small>
          {t("ep_query_params_of_cnt")}: [{epQueryParams.length}]
        </small>
      </strong>
    );
  }
          ...
          <div className={"container-fluid"}>
              ...
              <div className={"col-3"}>
                <div className="mb-1">
                  ...
                  <HttpMethodSelector
                    ...
                    disabled={prog}
                    data-testid="http-method-selector-test"
                  />
                </div>
              </div>
              <div className={"col-9"}>
            ...
            <div className={"row mt-2"}>
              <div className={"col"}>
                <div className={"row"}>
                  ...
                  <div className={"col"}>
                    <TextEditor
                      beautifyTrigger={beautifyCnt}
                      value={requestBody}
                      onChange={(v) => setRequestBody(v)}
                      onBlur={handleLeave}
                      disabled={isBodyDisabled() || prog}
                    />
                  </div>
                </div>
              </div>
            ...
            <div className={"row mt-2"}>
              <div className={"col-auto pe-1"}>
                <button
                  type="button"
                  className={"btn btn-rss btn-outline-secondary"}
                  onClick={runQuery}
                  disabled={prog}
                >
                  {t("btn_execute")}
