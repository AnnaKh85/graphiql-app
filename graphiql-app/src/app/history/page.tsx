...
export default function HistoryPage() {
  ...
  function loadData() {
    const list: HistorySavedItem[] = getAppLocalStorage().getHistory(
      authProps.userId ?? "",
    );
  ...
  function renderHistory(): ReactNode {
    return histItems.map(function (item) {
      ...
      const url = urlBuildService.buildUrl_fromHistory(
        item.type,
        item.payload.paramsBase64,
        item.payload.method,
        item.payload.url,
        item.payload.body ?? "",
      ...
      <div className={"card min-vw-90"}>
        <div className={"card-body"}>
          ...
          <div className={"container-fluid"}>
            <div className={"row"}>
              <div className={"col"}>{renderBody()}</div>
            </div>
          </div>
        </div>
      </div>
